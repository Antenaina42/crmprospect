...
export function ProspectDetailModal({ prospect, onClose, onUpdate }: ProspectDetailModalProps) {
  ...
  // Notes state
  const [notesText, setNotesText] = useState(prospect.notes || "");
  const [savingNotes, setSavingNotes] = useState(false);
  const [notesSavedSuccess, setNotesSavedSuccess] = useState(false);

  // Sync state if prospect prop updates
  useEffect(() => {
    setNotesText(prospect?.notes || "");
    setStatus(prospect?.status || "Nouveau");
    setPriority(prospect?.priority || "Moyenne");
  }, [prospect]);
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      const res = await fetch("/api/prospects", {
        ...
        body: JSON.stringify({ id: prospect.id, notes: notesText }),
      });
      if (res.ok) {
  ...
  const handleAppendTimestampedNote = (tag?: string) => {
    const now = new Date();
    ...
    const prefix = `[${dateStr}${tag ? ` - ${tag}` : ""}] : `;
    setNotesText((prev: string) => {
        ...
        <div className="p-4 sm:p-6 flex-1 overflow-y-auto space-y-4 sm:space-y-6 bg-slate-50/30">
          {activeTab === "overview" && (
            <>
              ...
              {/* Interactive Notes & Briefing Commercial Section */}
              <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                      ...
                      <span>Notes & Briefing Client / Prospect</span>
                    </span>
                    {notesSavedSuccess && (
                      <span className="text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full flex items-center gap-1 animate-pulse">
                  ...
                  <button
                    type="button"
                    onClick={() => handleAppendTimestampedNote("Note")}
                    className="px-2.5 py-1 bg-slate-100 hover:bg-brand-50 hover:text-brand-700 text-slate-700 rounded-lg text-[10px] font-bold flex items-center gap-1 transition-colors border border-slate-200"
                    title="Ajouter une ligne horodatée avec date et heure"
                  >
                ...
                <div className="space-y-2.5">
                  <textarea
                    ...
                    placeholder="Écrivez ici toutes les notes, besoins du client, points d'attention, montants négociés, détails de passation technique..."
                    className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all placeholder:text-slate-400 font-normal leading-relaxed resize-y"
                  />

                  <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                    {/* Quick tag snippets */}
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className="text-[10px] text-slate-400 font-medium">Ajout rapide :</span>
                      <button
                        type="button"
                        onClick={() => handleAppendTimestampedNote("Site Web")}
                        className="px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-200 rounded text-[10px] font-semibold hover:bg-blue-100 transition-colors"
                      >
                        + Site Web
                      </button>
                      <button
                        type="button"
                        onClick={() => handleAppendTimestampedNote("Devis Envoyé")}
                        className="px-2 py-0.5 bg-purple-50 text-purple-700 border border-purple-200 rounded text-[10px] font-semibold hover:bg-purple-100 transition-colors"
                      >
                        + Devis
                      </button>
                      <button
                        type="button"
                        onClick={() => handleAppendTimestampedNote("Acompte Versé")}
                        className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded text-[10px] font-semibold hover:bg-emerald-100 transition-colors"
                      >
                        + Acompte
                      </button>
                      <button
                        type="button"
                        onClick={() => handleAppendTimestampedNote("À Rappeler")}
                        className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded text-[10px] font-semibold hover:bg-amber-100 transition-colors"
                      >
                        + À Rappeler
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-xs font-bold shadow-xs flex items-center gap-1.5 transition-all active:scale-95 disabled:opacity-50 ml-auto"
                    >
                      {savingNotes ? (
                      ...
                      <span>{savingNotes ? "Enregistrement..." : "Enregistrer la Note"}</span>
                    </button>
                  </div>
                </div>
              </div>
